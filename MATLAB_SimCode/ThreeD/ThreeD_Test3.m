...
for k = 1:NS-1
    for x = 1:ND
        ...
        vx(k+1,x) = vx(k,x)*Acc_Ex(k,x)/(Acc_Ex(k,x) + (dz*vx(k,x)));
        vy(k+1,x) = vy(k,x)*Acc_Ey(k,x)/(Acc_Ey(k,x) + (dz*vy(k,x)));
        vz(k+1,x) = vz(k,x)*Acc_Ez(k,x)/(Acc_Ez(k,x) + (dz*vz(k,x)));
        tx(k+1,x) = (t(k,x) + dz/vx(k,x));
        ty(k+1,x) = (t(k,x) + dz/vy(k,x));
        tz(k+1,x) = (t(k,x) + dz/vz(k,x));
        posx(k,x) = t(k,x)*vx(k,x);
        posy(k,x) = t(k,x)*vy(k,x);
        posz(k,x) = t(k,x)*vz(k,x);
    end
    z(k+1) = z(k) + dz;
