...
%________________________________________________
%Switches - Specify as per Instructions          |
%________________________________________________|
% Velocity Modulation Switch                     |
% 0 - Velocity Modulation Off                    |
% 1 - Velocity Modulation On                     |
Vel_Mod_Switch = 0;             %Switch On/Off   |
%Space Charge Field Switch                       |
% 0 - Remove SCF from Tube                       |
% 1 - Keep SCF in Tube                           |
SCF_Switch = 0;                 %Switch On/Off   |
%________________________________________________|
...
%Variable Values                                 |
%________________________________________________|
F = 94000000000;   %Operating Frequency          |
I = 500;            %Beam Current in mA          |
V = 20.2;           %Beam Voltage in kV          |
BW= 21000000000;    %BandWidth of the RF Signal  |
Beam_T = 210;       %Beam Thickness in um        |
Beam_W = 1100;      %Beam Width in um            |
ND = 24;            %Number of Disc(or electrons)|
NZ = 16;            %Number of Steps per Lambda_e|
Ln = 0.1;           %Length of Tube in m         |
Pin= 0.08;          %Input Power of RF Signal    |
%________________________________________________|
...
for k = 1:NS-1
    for x = 1:ND
        ...
        Vin(k+1) = Vin(k)*exp(-1i*Beta*dz)+dVin;
...
DiscMass = abs((I/1000)*Lambda/(ND*u0*1.8*10^11));
