...
for k = 1:NS-1
    for i = 1:ND
        for j = 1:ND
            ...
            for l = 1:10
                for m = 1:10
                    E_rough = E_rough + (2*B_T_Height/(l*pi))*(2*W/(m*pi))*sin(m*pi*BeamW/(2*W))*((sin(m*pi/2))^2)*exp(-1*pi*m*sign(relpos(k,i,j))*relpos(k,i,j)/W)*sin(l*pi*BeamT/(2*B_T_Height)*((sin(l*pi/2))^2))*exp(-1*pi*l*sign(relpos(k,i,j))*relpos(k,i,j)/B_T_Height)*relpos(k,i,j);
                end
            end
        ...
        Acc_E(k+1,i) = (e*(SCF(k,i))/ms)*((1-(u0/c)^2)^(3/2))*((uine(k,i))^3)/10^12;
        %Acc_E(k+1,i) = 0;
...
for k = 1:NS
    for i = 1:ND
        Ec(k) = (E0*sin(Beta*Z(k))*sin(2*pi*F*t(k,i)));
    end
end
...
for k = 1:NS
    for x = 1:ND
        curr_const2(k) = curr_const2(k) + (sin(x*ang_f*BeamT*0.5*uine(k,x))/(x*ang_f*BeamT*0.5*uine(k,x)));
    end
end

for j=1:NS
    irf(j) = 100000*current_constant*curr_const2(k)*abs(sum(val(j,:)));
end
figure;
plot(z,irf/I0);
title('Current Representation at Different Planes due to Electron Bunching with Space Charge Field Applied');
