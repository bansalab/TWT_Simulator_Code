...
% Beam Tracking with/without Space Charge Field
...
for k = 1:NS-1
    ...
    z(k+1) = z(k) + dz;
    for x = 1:ND
        curr_const2 = curr_const2 + (sin(ang_f*BeamT*0.5*uine(k,x))/(ang_f*BeamT*0.5*uine(k,x)))*exp(-1i*ang_f*t(k,x));
    end
    irf(k+1) = 10*current_constant*curr_const2;
    curr_const2 = 0;
