...
% Calculating position of each electron in every plane
for k=1:NS
    for i=1:ND
        pos(k,i) = t(k,i)/uin(k,i);
    end
end

% Calculating the Relative Positions of each electron in different planes
E_rough = 0;
E_const = 2*I0*c*c/(pi*W*B_T_Height*BeamT*BeamW);
SCF(1:NS,1:ND) = 0;
...
for k=1:NS
    for i=1:ND
        for g=1:ND
            rel_pos(k,i,g) = pos(k,g) - pos(k,i);
            for l=1:10
                for m=1:10
                    E_rough = E_rough + (2*B_T_Height/(l*pi))*(2*W/(m*pi))*sin(m*pi*BeamW/(2*W))*((sin(m*pi/2))^2)*exp(-1*pi*m*sign(rel_pos(k,i,g))*rel_pos(k,i,g)/W)*sin(l*pi*BeamT/(2*B_T_Height)*((sin(l*pi/2))^2))*exp(-1*pi*l*sign(rel_pos(k,i,g))*rel_pos(k,i,g)/B_T_Height)*rel_pos(k,i,g);
                end
            end
            SCF_rough(k,i,g) = q*E_const*E_rough;
            E_rough = 0;
            SCF(k,i) = SCF(k,i) + SCF_rough(k,i,g);
        end
        d2tdz2(k,i) = SCF(k,i)/m;
    end
end