...
for limit = 1:1000
    for k = 1:NS-1
        ...
        dVin_N(k+1) = (25*Z/2)*abs(irf(k+1))*((1-exp(Beta*dz)/exp(-1*Tou_e*dz))/(1+Beta/Tou_e));
    end
    Vin2(NS) = Vin_N(NS) + dVin_N(NS);
    Ec(NS) = Beta*Vin2(NS);
    for k = 1:NS-1  
        Ec(NS-k) = (Ec(NS-k+1)*exp(1i*Beta*Ln/NS)) + (dVin_N(NS-k+1)*1i*Beta);
        Vin2(NS-k)= Ec(NS-k)/(1i*Beta);
    end
end

for k = 1:NS-1
    Pout_N(k) = abs(Vin2(k))*abs(Vin2(k))/(2*Z);
end
